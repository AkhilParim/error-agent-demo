...
// Errors: division-by-zero in conversion rate, typo in sort comparator
...
export function getMetricsSummary(): Metric[] {
  const signups = 124;
  // BUG 4: totalVisitors is hardcoded to 0 — division by zero produces Infinity
  // The value "Infinity%" renders visibly broken in the UI
  ...
  return [
    ...
    { label: "Conversion Rate", value: `${conversionRate.toFixed(2)}%`, change: -0.3, changeLabel: "vs last month", suffix: "%" },
...
// BUG 5: Typo 'reveneu' instead of 'revenue' — sorts by undefined, producing NaN comparisons
// Array order becomes unpredictable; top customer ranking is scrambled
export function getTopUsers(): User[] {
  ...
  return (users as UserWithTypo[]).sort((a, b) => (b.reveneu ?? 0) - (a.reveneu ?? 0));
}
